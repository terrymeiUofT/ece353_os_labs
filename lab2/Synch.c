...
-----------------------------  Mutex  ---------------------------------
  ...
  behavior Mutex
    ...
    --         on the mutex.
      ...
      method Init ()
	  sleepingThreads = new List [Thread]
          heldBy = null
        endMethod
	  ...
	  elseIf heldBy == currentThread
	      FatalError ("The thread cannot lock the Mutex because it is already holding a lock on the Mutex")
	  else
	      sleepingThreads.AddToEnd (currentThread)
	      currentThread.Sleep()
	      self.Lock()
	  endIf
          oldIntStat = SetInterruptsTo (oldIntStat)
	  ...
	  oldIntStat = SetInterruptsTo (DISABLED)
	  if heldBy == null
	      FatalError ("The current thread is trying to unlock a null Mutex")
	  elseIf heldBy != currentThread
	      FatalError ("The current thread cannot unlock the Mutex because it is not the holder")
	  else
	      t = sleepingThreads.Remove ()
	      if t
	          t.status = READY
	          readyList.AddToEnd (t)                 
	      endIf
